...
// Marking when each variable starts for convenience
...
class FG_eval {
 public:
  ...
  void operator()(ADvector& fg, const ADvector& vars) {
    ...
    // Multiply squared devation by 'cost factors in this case 3000' to adjust contribution of each deviation to cost
    for (int i = 0; i < N; i++) {
      fg[0] += 3000*CppAD::pow(vars[cte_start + i], 2);
      fg[0] += 3000*CppAD::pow(vars[epsi_start + i], 2);
      fg[0] += 0.3 * CppAD::pow(vars[v_start + i] - ref_v, 2);
    }
    ...
    //
    // Setting up  Constraints for state variables and actuators
...
vector<double> MPC::Solve(Eigen::VectorXd state, Eigen::VectorXd coeffs) {
  ...
  //  Setting up  the number of model variables (includes both states and inputs).
